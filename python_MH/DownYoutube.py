...
for number in range(len(sample_df)):
    ...
    yt = YouTube(par)
    yt.title = sample_df.iloc[number, 1] # 파일 이름을 노래 제목으로 지정
    yt.streams.filter(only_audio=True).all() # 유튜브 정보 가져오기
    yt.streams.filter(only_audio=True).first().download() # mp4 형식으로 유튜브 영상 다운
    mp4_file = base_dir + "{}.mp4".format(sample_df.iloc[number, 1]) # mp4 파일 선택
    wav_file = base_dir + "{}.wav".format(sample_df.iloc[number, 1]) # wav 파일 만들기
    cmd = "ffmpeg -i {} -vn {}".format(mp4_file, wav_file) # mp4를 wav로 변환
    os.system(cmd) # 실행
    os.remove(mp4_file) # mp4 파일 제거
    song = AudioSegment.from_wav("{}.wav".format(sample_df.iloc[number, 1])) # wav 파일 선택
    five_seconds = 5 * 1000 # 5초
    first_5_seconds = song[:five_seconds] # 앞부분 5초 자르기
    first_5_seconds.export("{}.wav".format(sample_df.iloc[number, 1]), format="wav") # wav파일 다시저장
